const ALLOWED_ORIGINS=new Set(['https://superleague.streamlinewebapps.com','https://sly-api.luckdragon.io','http://localhost:3000','http://localhost:8788']);
const corsHeaders=(req)=>{const o=req.headers.get('Origin')||'';const ok=ALLOWED_ORIGINS.has(o)?o:'https://superleague.streamlinewebapps.com';return{'Access-Control-Allow-Origin':ok,'Access-Control-Allow-Methods':'GET,POST,PATCH,DELETE,PUT,OPTIONS','Access-Control-Allow-Headers':'Content-Type,Authorization,Accept,X-Coach-Id,X-Coach-Pin,X-Pin','Access-Control-Max-Age':'86400','Vary':'Origin'}};
const H={'Content-Type':'application/json','Access-Control-Allow-Origin':'https://superleague.streamlinewebapps.com','Access-Control-Allow-Methods':'GET,POST,PATCH,DELETE,PUT,OPTIONS','Access-Control-Allow-Headers':'Content-Type,Authorization,Accept,X-Coach-Id,X-Coach-Pin,X-Pin','Access-Control-Max-Age':'86400','Vary':'Origin'};
...
// HMAC-SHA256 helpers for signed session tokens. Format: base64url(coachId:exp).base64url(hmacSig)
const _b64u=(buf)=>btoa(String.fromCharCode(...new Uint8Array(buf))).replace(/\+/g,'-').replace(/\//g,'_').replace(/=+$/,'');
const _b64uDec=(s)=>{s=s.replace(/-/g,'+').replace(/_/g,'/');while(s.length%4)s+='=';return atob(s)};
const signSession=async(secret,coachId,ttlSec=86400*7)=>{const exp=Math.floor(Date.now()/1000)+ttlSec;const payload=`${coachId}:${exp}`;const k=await crypto.subtle.importKey('raw',new TextEncoder().encode(secret),{name:'HMAC',hash:'SHA-256'},false,['sign']);const sig=await crypto.subtle.sign('HMAC',k,new TextEncoder().encode(payload));return `${_b64u(new TextEncoder().encode(payload))}.${_b64u(sig)}`};
const verifySession=async(secret,token)=>{if(!token||typeof token!=='string'||!token.includes('.'))return null;const[hp,hs]=token.split('.');try{const payload=_b64uDec(hp);const[cid,exp]=payload.split(':');if(!cid||!exp)return null;if(Date.now()/1000>+exp)return null;const k=await crypto.subtle.importKey('raw',new TextEncoder().encode(secret),{name:'HMAC',hash:'SHA-256'},false,['verify']);const sigBytes=Uint8Array.from(_b64uDec(hs),c=>c.charCodeAt(0));const ok=await crypto.subtle.verify('HMAC',k,sigBytes,new TextEncoder().encode(payload));return ok?{coach_id:+cid,exp:+exp}:null}catch(e){return null}};

// HMAC-SHA256 hash for PINs (uses SLY_SESSION_SECRET as key). Hex-encoded.
const hashPin=async(secret,pin)=>{const k=await crypto.subtle.importKey('raw',new TextEncoder().encode(secret),{name:'HMAC',hash:'SHA-256'},false,['sign']);const sig=await crypto.subtle.sign('HMAC',k,new TextEncoder().encode(String(pin)));return Array.from(new Uint8Array(sig)).map(b=>b.toString(16).padStart(2,'0')).join('')};
const verifyPin=async(secret,pin,hash,plaintext_fallback)=>{if(hash){const h=await hashPin(secret,pin);return h===hash}return plaintext_fallback&&String(plaintext_fallback)===String(pin)};
const requireCoachAuth=async(req,db,expectedCoachId=null,env=null)=>{
  // Bearer signed-session token (preferred)
  const auth=(req.headers.get('Authorization')||'').replace(/^Bearer\s+/i,'');
  if(auth&&env&&env.SLY_SESSION_SECRET){const v=await verifySession(env.SLY_SESSION_SECRET,auth);if(v){if(expectedCoachId!==null&&v.coach_id!==+expectedCoachId)return null;return{id:v.coach_id}}}
  // Legacy header fallback
  const cid=+(req.headers.get('X-Coach-Id')||'');const pin=req.headers.get('X-Coach-Pin')||'';if(!cid||!pin)return null;
  const c=await db.prepare('SELECT id FROM coaches WHERE id=? AND pin=?').bind(cid,String(pin)).first();if(!c)return null;
  if(expectedCoachId!==null&&c.id!==+expectedCoachId)return null;return c};
const requireAdmin=async(req,env,db)=>{const auth=(req.headers.get('Authorization')||'').replace('Bearer ','');const tok=env.MIGRATION_TOKEN||'';if(tok&&auth===tok)return{id:0,role:'system'};const xpin=req.headers.get('X-Pin')||'';if(env.PADDY_PIN&&xpin===env.PADDY_PIN)return{id:0,role:'system'};const c=await requireCoachAuth(req,db,null,env);if(c&&c.id===1)return{...c,role:'admin'};return null};
...
if(p==='/api/coaches/login'&&m==='POST'){const{coach_id,pin}=await R(req);if(!coach_id||!pin)return E('coach_id and pin required');const row=await db.prepare('SELECT id,name,team_name,color,avatar_emoji,logo_url,pin AS plaintext_pin,pin_hash FROM coaches WHERE id=?').bind(coach_id).first();if(!row)return E('Invalid credentials',401);const ok=env.SLY_SESSION_SECRET?await verifyPin(env.SLY_SESSION_SECRET,pin,row.pin_hash,row.plaintext_pin):String(row.plaintext_pin)===String(pin);if(!ok)return E('Invalid credentials',401);// Lazy-migrate: if hash missing, set it now while we have the plaintext PIN
if(env.SLY_SESSION_SECRET&&!row.pin_hash){const h=await hashPin(env.SLY_SESSION_SECRET,pin);await db.prepare('UPDATE coaches SET pin_hash=? WHERE id=?').bind(h,coach_id).run()}delete row.plaintext_pin;delete row.pin_hash;const sessionToken=env.SLY_SESSION_SECRET?await signSession(env.SLY_SESSION_SECRET,row.id):null;return J({ok:true,coach:row,token:sessionToken||btoa(`${row.id}:${Date.now()}`),session_token:sessionToken,expires_in:86400*7})}
const cm=p.match(/^\/api\/coaches\/(\d+)$/);if(cm){const id=+cm[1];if(m==='GET'){const row=await db.prepare('SELECT id,name,team_name,color,avatar_emoji,logo_url,email FROM coaches WHERE id=?').bind(id).first();if(!row)return E('Not found',404);return J(row)}if(m==='PATCH'){if(!await requireCoachAuth(req,db,id,env)&&!await requireAdmin(req,env,db))return E401();const b=await R(req);const a=['team_name','color','avatar_emoji','logo_url','email'];const s=[],v=[];for(const k of a)if(k in b){s.push(`${k}=?`);v.push(b[k])}if(!s.length)return E('No fields');v.push(id);await db.prepare(`UPDATE coaches SET ${s.join(',')} WHERE id=?`).bind(...v).run();return J({ok:true})}}
const pinm=p.match(/^\/api\/coaches\/(\d+)\/pin$/);if(pinm&&m==='PATCH'){const id=+pinm[1];const{current_pin,new_pin}=await R(req);if(!new_pin||String(new_pin).length<4)return E('PIN must be at least 4 chars');const cur=await db.prepare('SELECT pin AS plaintext_pin,pin_hash FROM coaches WHERE id=?').bind(id).first();if(!cur)return E('Coach not found',404);const ok=env.SLY_SESSION_SECRET?await verifyPin(env.SLY_SESSION_SECRET,current_pin,cur.pin_hash,cur.plaintext_pin):String(cur.plaintext_pin)===String(current_pin);if(!ok)return E('Current PIN incorrect',401);const newHash=env.SLY_SESSION_SECRET?await hashPin(env.SLY_SESSION_SECRET,new_pin):null;await db.prepare('UPDATE coaches SET pin=?,pin_hash=? WHERE id=?').bind(String(new_pin),newHash,id).run();return J({ok:true})}
// FIXED: /api/rounds returns ALL rounds (incl HS + Final) so SPA can render full season structure. Was clipping at current+1.
if(p==='/api/rounds'&&m==='GET'){const{results}=await db.prepare('SELECT id,name,round_number,is_complete,lock_time,round_type FROM rounds ORDER BY round_number').all();return new Response(JSON.stringify(results.map(addStatus)),{headers:{...H,'Cache-Control':'no-cache, must-revalidate, max-age=0'}})}
...
const plm=p.match(/^\/api\/players\/([\w-]+)$/);if(plm){if(m==='GET'){const row=await db.prepare('SELECT * FROM players WHERE id=?').bind(plm[1]).first();if(!row)return E('Not found',404);return J(row)}if(m==='PATCH'){if(!await requireAdmin(req,env,db))return E403();const b=await R(req);const a=['name','team','position','photo_url','coach_id','champid'];const s=[],v=[];for(const k of a)if(k in b){s.push(`${k}=?`);v.push(b[k])}if(!s.length)return E('No fields');v.push(plm[1]);await db.prepare(`UPDATE players SET ${s.join(',')} WHERE id=?`).bind(...v).run();return J({ok:true})}}
// FIXED: /api/picks accepts round_id and coach_id; filters on rp.round_id directly
// ── Cron observability — POST /api/cron/heartbeat (admin), GET /api/cron/last-runs (public) ──
if(p==='/api/cron/heartbeat'&&m==='POST'){if(!await requireAdmin(req,env,db))return E403();const b=await R(req);const{cron_name,status='ok',message=null}=b;if(!cron_name)return E('cron_name required');await db.prepare("INSERT INTO cron_runs (cron_name,last_run_at,last_status,last_message,run_count) VALUES (?,datetime('now'),?,?,1) ON CONFLICT(cron_name) DO UPDATE SET last_run_at=datetime('now'),last_status=excluded.last_status,last_message=excluded.last_message,run_count=cron_runs.run_count+1").bind(cron_name,status,message).run();return J({ok:true,cron_name,recorded_at:new Date().toISOString()})}
if(p==='/api/cron/last-runs'&&m==='GET'){const{results}=await db.prepare('SELECT cron_name,last_run_at,last_status,last_message,run_count FROM cron_runs ORDER BY cron_name').all();const now=Date.now();const enriched=results.map(r=>{const ageS=Math.floor((now-new Date(r.last_run_at+'Z').getTime())/1000);let stale=false;if(r.cron_name==='sly-score-cron'&&ageS>120)stale=true;if(r.cron_name==='sly-autopick-cron'&&ageS>1200)stale=true;return{...r,age_s:ageS,stale}});return new Response(JSON.stringify(enriched),{headers:{...H,'Cache-Control':'no-cache, must-revalidate, max-age=0'}})}
// ── Forgot-PIN flow — POST /api/coaches/forgot-pin (request reset by email), POST /api/coaches/reset-pin (consume token + new pin) ──
if(p==='/api/coaches/forgot-pin'&&m==='POST'){const b=await R(req);const{email}=b;if(!email)return E('email required');const c=await db.prepare('SELECT id,name,email FROM coaches WHERE email=? COLLATE NOCASE').bind(email).first();if(!c){await new Promise(r=>setTimeout(r,400));return J({ok:true,message:'If an account exists for that email, a reset link has been sent'})}const tok=Array.from(crypto.getRandomValues(new Uint8Array(24))).map(b=>b.toString(16).padStart(2,'0')).join('');const expires=new Date(Date.now()+30*60*1000).toISOString();await db.prepare('INSERT INTO pin_reset_tokens (token,coach_id,expires_at,used) VALUES (?,?,?,0)').bind(tok,c.id,expires).run();const link=`https://superleague.streamlinewebapps.com/?reset_token=${tok}`;if(env.RESEND_API_KEY){const emailRes=await fetch('https://api.resend.com/emails',{method:'POST',headers:{'Authorization':'Bearer '+env.RESEND_API_KEY,'Content-Type':'application/json'},body:JSON.stringify({from:'SLY <noreply@send.luckdragon.io>',to:[c.email],subject:'Reset your SLY PIN',html:`<p>Hi ${c.name},</p><p>Click the link below to set a new PIN. Expires in 30 min.</p><p><a href="${link}">${link}</a></p><p>If you didn't request this, ignore this email — your PIN won't change.</p>`})});const ej=await emailRes.json().catch(()=>({}));if(!emailRes.ok)return E('Email send failed: '+JSON.stringify(ej),502)}return J({ok:true,message:'Reset link sent to '+c.email})}
if(p==='/api/coaches/reset-pin'&&m==='POST'){const b=await R(req);const{token,new_pin}=b;if(!token||!new_pin)return E('token and new_pin required');if(String(new_pin).length<4)return E('PIN must be at least 4 chars');const row=await db.prepare("SELECT coach_id,expires_at,used FROM pin_reset_tokens WHERE token=?").bind(token).first();if(!row)return E('Invalid token',401);if(row.used)return E('Token already used',410);if(new Date(row.expires_at)<new Date())return E('Token expired',410);const newHashR=env.SLY_SESSION_SECRET?await hashPin(env.SLY_SESSION_SECRET,new_pin):null;await db.prepare('UPDATE coaches SET pin=?,pin_hash=? WHERE id=?').bind(String(new_pin),newHashR,row.coach_id).run();await db.prepare('UPDATE pin_reset_tokens SET used=1 WHERE token=?').bind(token).run();return J({ok:true,message:'PIN updated. Sign in with your new PIN.'})}
if(p==='/api/picks'){if(m==='GET'){const rd=+u.searchParams.get('round_id')||0,rn=+u.searchParams.get('round')||0,co=+u.searchParams.get('coach_id')||+u.searchParams.get('coach')||0;let q='SELECT rp.id,rp.coach_id,r.round_number,r.id AS round_id,rp.player_id,rp.slot,rp.banter,rp.created_at FROM round_picks rp JOIN rounds r ON r.id=rp.round_id WHERE 1=1';const ps=[];if(rd){q+=' AND rp.round_id=?';ps.push(rd)}else if(rn){q+=' AND r.round_number=?';ps.push(rn)}if(co){q+=' AND rp.coach_id=?';ps.push(co)}q+=' ORDER BY rp.coach_id,rp.slot';const{results}=await db.prepare(q).bind(...ps).all();return J(results)}if(m==='POST'){const b=await R(req);const rn=+b.round_number,ci=+b.coach_id;if(!await requireCoachAuth(req,db,ci,env)&&!await requireAdmin(req,env,db))return E401();if(!rn||!ci||!Array.isArray(b.picks))return E('round_number,coach_id,picks[] required');const r=await db.prepare('SELECT id,lock_time FROM rounds WHERE round_number=?').bind(rn).first();if(!r)return E('Round not found',404);if(r.lock_time&&new Date(r.lock_time)<new Date())return E('Round is locked',423);await db.prepare('DELETE FROM round_picks WHERE round_id=? AND coach_id=?').bind(r.id,ci).run();for(const pk of b.picks){await db.prepare('INSERT INTO round_picks (coach_id,round_id,player_id,slot,banter) VALUES (?,?,?,?,?)').bind(ci,r.id,pk.player_id,pk.slot,pk.banter||'').run()}return J({ok:true,count:b.picks.length})}}
...
if(p==='/api/scores'&&m==='GET'){const rd=+u.searchParams.get('round')||+u.searchParams.get('round_id')||0;let q='SELECT s.id,s.coach_id,r.id AS round_id,r.round_number,s.points,s.opponent_id,s.result,s.match_name,COALESCE(s.points_against,0) AS points_against,COALESCE(s.max_score,0) AS max_score FROM scores s JOIN rounds r ON r.id=s.round_id';const ps=[];if(rd){const useRoundId=new URL(req.url).searchParams.has('round_id');if(useRoundId){q+=' WHERE s.round_id=?';ps.push(rd)}else{q+=' WHERE r.round_number=?';ps.push(rd)}}q+=' ORDER BY r.round_number,s.points DESC';const{results}=await db.prepare(q).bind(...ps).all();return new Response(JSON.stringify(results),{headers:{...H,'Cache-Control':'no-cache, must-revalidate, max-age=0'}})}
...
if(p==='/api/coaches/auto_pick'&&m==='PATCH'){
  ...
  if(!await requireCoachAuth(req,db,b.coach_id,env)&&!await requireAdmin(req,env,db))return E401();
  await db.prepare('UPDATE coaches SET auto_pick_enabled=? WHERE id=?').bind(b.enabled?1:0,b.coach_id).run();
  return J({ok:true})
}
if(p==='/api/_admin/d1-dump'&&m==='GET'){if(!await requireAdmin(req,env,db))return E403();const tables=['coaches','rounds','sly_fixtures','round_picks','scores','players','payments','config','cron_runs','injury_list','activity_feed','draft_picks'];const dump={};for(const t of tables){try{const{results}=await db.prepare('SELECT * FROM '+t).all();dump[t]=results}catch(e){dump[t]={error:String(e)}}}return new Response(JSON.stringify({generated_at:new Date().toISOString(),tables:Object.keys(dump),row_counts:Object.fromEntries(Object.entries(dump).map(([k,v])=>[k,Array.isArray(v)?v.length:'err'])),data:dump},null,0),{headers:{...H,'Content-Type':'application/json','Cache-Control':'no-store'}})}
if(p==='/api/_admin/migrate-pins'&&m==='POST'){if(!await requireAdmin(req,env,db))return E403();if(!env.SLY_SESSION_SECRET)return E('SLY_SESSION_SECRET not bound',500);const{results}=await db.prepare('SELECT id,pin FROM coaches WHERE pin_hash IS NULL').all();let migrated=0;for(const c of results){if(!c.pin)continue;const h=await hashPin(env.SLY_SESSION_SECRET,c.pin);await db.prepare('UPDATE coaches SET pin_hash=? WHERE id=?').bind(h,c.id).run();migrated++}return J({ok:true,migrated,scanned:results.length})}
...
const paym=p.match(/^\/api\/payments\/(\d+)$/);if(paym&&m==='PATCH'){if(!await requireAdmin(req,env,db))return E403();const ci=+paym[1];const b=await R(req);const s=[],v=[];for(const k of ['paid','amount','note','tier','gold_balance','autopick_paid'])if(k in b){s.push(`${k}=?`);v.push(b[k])}if(!s.length)return E('No fields');s.push("updated_at=datetime('now')");v.push(ci);await db.prepare('INSERT OR IGNORE INTO payments (coach_id,paid,amount) VALUES (?,0,50)').bind(ci).run();await db.prepare(`UPDATE payments SET ${s.join(',')} WHERE coach_id=?`).bind(...v).run();return J({ok:true})}
...
const tradm=p.match(/^\/api\/trades\/(\d+)$/);if(tradm&&m==='PATCH'){if(!await requireAdmin(req,env,db))return E403();const id=+tradm[1];const b=await R(req);if(!b.status)return E('status required');await db.prepare('UPDATE trades SET status=? WHERE id=?').bind(b.status,id).run();return J({ok:true})}
...
if(p.startsWith('/api/sly-fixtures')){
  ...
  if(sfm&&m==='PATCH'){if(!await requireAdmin(req,env,db))return E403();const id=+sfm[1];const b=await R(req);const s=[],v=[];for(const k of ['match_name'])if(k in b){s.push(`${k}=?`);v.push(b[k])}if(!s.length)return E('No fields');v.push(id);await db.prepare(`UPDATE sly_fixtures SET ${s.join(',')} WHERE id=?`).bind(...v).run();return J({ok:true})}
  if(m==='GET'){
