...
if(p==='/api/coaches/login'&&m==='POST'){const{coach_id,pin}=await R(req);if(!coach_id||!pin)return E('coach_id and pin required');const row=await db.prepare('SELECT id,name,team_name,color,avatar_emoji,logo_url FROM coaches WHERE id=? AND pin=?').bind(coach_id,String(pin)).first();if(!row)return E('Invalid credentials',401);const sessionToken=env.SLY_SESSION_SECRET?await signSession(env.SLY_SESSION_SECRET,row.id):null;return J({ok:true,coach:row,token:sessionToken||btoa(`${row.id}:${Date.now()}`),session_token:sessionToken,expires_in:86400*7})}
